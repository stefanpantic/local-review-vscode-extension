...
If a client is refused because it was registered for another window's server, ReviewMate warns you and offers the command that registers this window's server. Run it, then reconnect the client (in Claude Code, `/mcp` or a new session).
...
You do not keep one global list of servers. The `claude mcp add` command we generate uses Claude Code's default **local scope**, which is tied to the directory it was added from, so the command changes into the right one first. Claude Code in a VS Code window starts in the window's first workspace folder, so that is where the command registers the window's server. In a multi-root window that one registration covers every repository in the window. If you reorder the folders, run the command again. A repository open in two windows (on its own and in a multi-root workspace) points at whichever window registered last. Cursor and VS Code's own MCP support have the same per-project scoping. The exception is clients with a single global config and no project scope (for example Claude Desktop), where you give each server a distinct name. In every case you only wire up the repositories you actually want the agent to review.
