...
- Gates must pass before pushing (CI runs the same): `pnpm run format:check`, `lint`, `typecheck`, `test`, `build`, `package`. Setup and the F5 dev loop are in [CONTRIBUTING.md](CONTRIBUTING.md).
...
Once connected, these tools are available. In a workspace with several repositories, every tool except `list_repos` takes an optional `repo` (a name or path from `list_repos`). A tool that changes the review (`post_comment`, `reply`, `resolve`, `react`, `edit_comment`, `delete_comment`) requires `repo` when several repositories are open. A read without it acts on the only repository, else the one whose review panel the human focused last, and otherwise errors with the list.

- `list_repos` — the repositories open in the workspace, the diff each shows, and which one is the default.
...
- `post_comment` — add a comment on a line or range (`side` "new" for added/context lines, "old" for removed), optionally with a `suggestion`. Omit `startLine` and `side` to comment on the whole file (no `suggestion` there).
- `reply` / `resolve` — respond in a thread or resolve/reopen it.
- `react` — toggle an emoji reaction (👍 👎 👀 ❤️ 🎉) on any comment, addressed by thread id and comment id.
