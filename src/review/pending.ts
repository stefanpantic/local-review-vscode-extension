...
import { hasReactionDiff } from '../model/Comment';
...
export interface PendingSummary {
  ...
  reactions: number; // comments with changed reactions
  total: number;
}

const EMPTY: PendingSummary = { newComments: 0, resolvedToggles: 0, edits: 0, deletes: 0, reactions: 0, total: 0 };
...
export function pendingChangeSet(review: Review): PendingSummary {
  ...
  for (const t of review.threads) {
    ...
    for (const c of t.comments) {
      ...
      if (c.remoteId && hasReactionDiff(c)) reactions++;
    }
  }
  const deletes = review.pendingDeletes?.length ?? 0;
  const total = newComments + resolvedToggles + edits + deletes + reactions;
  return { newComments, resolvedToggles, edits, deletes, reactions, total };
}
