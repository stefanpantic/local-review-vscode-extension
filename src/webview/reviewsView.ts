...
export class ReviewsView implements vscode.TreeDataProvider<ReviewNode> {
  ...
  getTreeItem(node: ReviewNode): vscode.TreeItem {
    if (isGroup(node)) {
      ...
      if (node.archived) item.tooltip = 'This branch no longer exists — move a review to your current branch to reuse it.';
      item.contextValue = 'localReview.branchGroup';
      return item;
    }
    const review = node;
    const isCurrent = review.id === this.controller.currentReviewId() && review.branch === this.controller.currentBranch();
