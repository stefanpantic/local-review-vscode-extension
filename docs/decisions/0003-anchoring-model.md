...
A **diff hash is never part of a key**. `source` is advisory provenance, not a partition key (see [ADR-0004](./0004-state-ownership.md) and [spec.md §7](../spec.md#7-data--storage-model-overview)). The _same_ engine re-anchors saved reviews on load.
