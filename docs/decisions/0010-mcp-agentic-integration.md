...
- **Local-only + opt-in.** Bound to `127.0.0.1`, guarded by a bearer token, **off by default** — autostart on launch is opt-in (`localReview.mcp.autoStart`), and Start/Stop commands control it on demand. A "Set up MCP for Claude Code" command prompts for the port and autostart, persists the port so the URL survives restarts, writes the connection into `.local-review/` (gitignored), and copies the `claude mcp` one-liner. Never binds a non-loopback address — "nothing leaves the box" holds.
