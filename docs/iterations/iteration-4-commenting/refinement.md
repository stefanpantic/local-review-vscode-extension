...
- **Awaited mutations** over the existing `id`-correlated `request()` bridge (this is the "awaited-mutation helper" [ADR-0004](../../decisions/0004-state-ownership.md) deferred to it.4 — no new bridge primitive; a mutation _is_ a request that resolves with the canonical thread).
...
- **Source-switch semantics** are the subtle correctness case: threads keyed by `repoRoot` only, re-anchored against whatever diff is loaded. This is _why_ the key excludes `source`; AC10 guards it explicitly.
