...
- [x] **AC9 — CI runs on a PR.** Opening a PR triggers `ci.yml`: install → format:check → lint → typecheck → test → build → package; the `.vsix` artifact is attached to the run; the workflow is green. _(Verified: CI has gated every PR since the first push; 78+ PRs merged.)_
