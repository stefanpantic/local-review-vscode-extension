...
- [ ] **AC1 — Auto-refresh on working-tree change.** Editing/saving a file updates the diff (and re-anchors comments) within a debounce, no manual Refresh.
- [ ] **AC2 — Auto-refresh on branch switch.** `git switch`/checkout updates the panel and the Reviews/Comments trees to the new branch's current review.
- [ ] **AC3 — Coalesced.** A burst (checkout, rebase, save-all) triggers a single refresh, not a storm; typing in an unsaved buffer doesn't thrash (fires on the git-visible change).
- [ ] **AC4 — Intra-line highlight.** A modified line highlights only its changed spans (stronger green/red) over the syntax colors, in **both** unified and split; the whole-line add/del tint remains underneath.
- [ ] **AC5 — Only modifications.** Pure added / removed / context lines get no intra-line marks; a run of N deletes + M adds pairs up sensibly.
- [ ] **AC6 — Keyboard nav.** Commands (with keybindings) jump to next/previous **changed file** and next/previous **comment**; focus/scroll follows.
- [ ] **AC7 — Large-file guard.** A file above the threshold renders collapsed with a "Load anyway" that expands it on demand.
- [ ] **AC8 — Expand context.** Hunk-boundary expanders reveal unchanged lines above/below a hunk (from the full file, with correct line numbers), bounded by the neighbouring hunk / file edges; no expander when there's nothing to reveal.
- [ ] **AC9 — Virtualization (conditional).** Measure a large diff (e.g. thousands of rows); if scrolling/DOM is slow, windowing keeps the DOM bounded and smooth; otherwise record the measurement and skip. Either way, no regression.
- [x] **AC10 — Green gates.** `build`, `typecheck`, `test`, `lint` pass; `wordDiff` has unit coverage. *(build + typecheck + 63/63 tests + lint; `test/wordDiff.test.ts` covers change/insert/delete/identical/prefix-suffix/whitespace.)*

**Verification status.** Automated checks PASS (AC10 ✓). **AC1–AC8 require a manual `F5` session** (steps in [`notes.md`](./notes.md)). **AC9 (virtualization) is a measurement**: scroll a large diff during F5 — build windowing only if it's janky; otherwise record that the eager renderer suffices.
...
- **Virtualization**: *measure*; implement behind the row model only if needed.
