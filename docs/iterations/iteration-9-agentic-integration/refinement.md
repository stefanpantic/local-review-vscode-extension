...
- **D5 — Local-only + opt-in, with explicit lifecycle.** The server binds **127.0.0.1** only, is guarded by a **token**, and is **off by default**. Autostart on launch is opt-in (`localReview.mcp.autoStart`); **Start MCP Server** / **Stop MCP Server** control it on demand (Start runs setup first if never configured). A **"Set up MCP for Claude Code"** command prompts for the port + autostart, persists the port (stable URL across restarts), writes `.local-review/mcp.json` (gitignored), and copies an idempotent `claude mcp` one-liner. A localhost server doesn't change "nothing leaves the box."
...
- [ ] **AC1 — Server + discovery + lifecycle.** The setup command prompts for port + autostart and produces a working `claude mcp` registration; the server serves on `127.0.0.1:<port>`; Claude Code connects and lists the tools. Start/Stop commands and `mcp.autoStart` (on launch) control it; the port persists so the URL survives restarts.
