...
- **Entry points**: an **Export** button in the diff panel's editor title bar (`editor/title` when `activeWebviewPanelId == localReview.panel`) — the most discoverable, right where you review; plus a title icon on the _Active review comments_ panel (current review) and a right-click **Export Review** on any _Reviews_ item. (A **Refresh** button rides in the panel title bar too, until it.7 auto-refresh.) Host-side only; no new webview messages.
