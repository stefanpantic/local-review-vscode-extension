...
| 12  | **Write your review back**        | Stage all local work on a PR (new comments, replies, resolve/unresolve, edits, deletes) as a **pending change set** and post it to GitHub on a single explicit **Submit** (chosen event), pinned to the reviewed head sha. The one egress; the posted set stays pure-read until then, so drafts never conflate. Local edit/delete only on your own or AI-authored comments. github.com + GHE exercised throughout. _Detailed refinement when opened._                                                                                                                         |

Each row links to its folder under [`iterations/`](./iterations/) once refined. Rows 10 and 12 carry intent and the central open decision only; their detailed refinement is written when the iteration opens (one iteration at a time).
