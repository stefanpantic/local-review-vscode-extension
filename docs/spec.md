...
| 9   | **Agentic integration**           | Two-way bridge so a coding agent (e.g. Claude Code) can **fetch** the current/any review as structured data _and_ **post** review comments back into Local Review (anchored like human comments, visible in the UI, repliable/resolvable). Central open decision: the transport + whether reviews gain an on-disk representation a separate process can read/write (touches invariant 3). Nothing is ever auto-applied to code (§3 holds). _Detailed refinement when opened._ |
