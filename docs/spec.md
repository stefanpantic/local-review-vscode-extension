...
**Status: rows 1 through 9, 4b, and 11 through 14 have shipped. Row 10 (scale-out) is the only one left; its refinement is a parked draft, and no iteration is currently open.** Each shipped iteration's `refinement.md` carries its ticked acceptance criteria as the verification record.
