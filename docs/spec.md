...
- **Agent-ready structured export** as well-structured **Markdown**, or as versioned **JSON** for scripts and tools.
...
6. **Export Review** → a structured Markdown or JSON work list, copied to the clipboard, opened in an editor, or saved to a file.
7. Paste the Markdown into a coding agent to act on the comments, or feed the JSON to your own tooling. Iterate: as the agent edits code, comments **drift** with their lines or become _outdated_.
...
| 10  | **Scale-out testing** _(parked)_  | Stress the whole surface at scale — thousands of files / very large diffs, many comments and reviews — and measure render/scroll/refresh/anchoring/export. Settles the it.7-deferred **windowed virtualization** definitively (build it if the measurements demand it); checks watcher cost on huge trees and memory footprint. Likely a fixture generator for synthetic large repos. _Parked: refinement is a draft; will be opened if real-world scale problems surface._                                                                                                                                       |
| 11  | **Review a GitHub PR (read)**     | Point the extension at a GitHub PR (list, or URL/number) and review its diff and all review threads in the same UI, **in place** — fetch head+base into hidden refs, never checkout. A `'pr'` diff source (three-dot `base...head`) on the existing CLI + `normalize` pipeline; a neutral `ReviewProvider` seam with a GitHub-only implementation; VS Code native auth (transient token); github.com + GitHub Enterprise. Imported threads render as-is (resolved/outdated) and are pure-read; MCP reads a PR for free. See [ADR-0011](./decisions/0011-github-pr-review.md).                                     |
| 12  | **Write your review back**        | Stage all local work on a PR (new comments, replies, resolve/unresolve, edits, deletes) as a **pending change set** and post it to GitHub on a single explicit **Submit** (chosen event), pinned to the reviewed head sha. The one egress; the posted set stays pure-read until then, so drafts never conflate. Local edit/delete only on your own or AI-authored comments. github.com + GHE exercised throughout. See [`iterations/iteration-12-github-pr-writeback/`](./iterations/iteration-12-github-pr-writeback/refinement.md).                                                                             |
...
| 18  | **JSON export**                   | **Export Review** offers JSON next to Markdown, for scripts and custom agent harnesses. A format step comes first, then the same scope, line-reference, and target steps. A pure `exportReviewJson` formatter shares scope filtering and ordering with the Markdown formatter. The JSON has a `version` field and adds comment ids, authors, suggestion originals, and reactions. MCP output doesn't change. See [`iterations/iteration-18-json-export/`](./iterations/iteration-18-json-export/refinement.md).                                                                                                   |
| 19  | **Multi-root workspaces**         | No active repository. Every sidebar view spans all repositories in the workspace, one section per repository, flat when there is one. Each repository has its own session (source, base ref, open PR, current review, poll, panel). Commands take their repository from the item, focused panel, or active editor, and ask only when ambiguous. The PR list covers every GitHub repository with a `repo:` filter token. MCP tools take an optional `repo`. See [ADR-0012](./decisions/0012-workspace-wide-views.md) and [`iterations/iteration-19-multi-root/`](./iterations/iteration-19-multi-root/refinement.md). |
| 20  | **Nested repositories**           | Discover git repositories and submodules below a workspace folder, so each gets its own section and session. _Not yet refined._ |

Each row links to its folder under [`iterations/`](./iterations/) once refined. Rows 10 and 20 carry intent only; its detailed refinement is written when the iteration opens (one iteration at a time).

**Status: rows 1 through 9, 4b, and 11 through 18 have shipped. Row 19 (multi-root workspaces) is open. Row 20 follows it. Row 10 (scale-out) is parked indefinitely — its refinement is a draft, and it will only be opened if real-world scale problems surface.** Each shipped iteration's `refinement.md` carries its ticked acceptance criteria as the verification record.
...
| [0011](./decisions/0011-github-pr-review.md)               | Review GitHub PRs locally, in place (opt-in remote)        |
| [0012](./decisions/0012-workspace-wide-views.md)           | Workspace-wide views, one session per repository           |
...
- **Export** — the external Markdown or JSON generated from a review.
